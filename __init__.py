...
for file in files:
    ...
    if os.path.exists(input("Enter file:")):
        shutil.move(input("Enter file:"))
    else:
        os.chdir()
        os.getcwd()
        os.makedirs(input("folder name:"))
        os.remove(input("Enter file name:"))  # error if not found
        os.rmdir(input("folder name:")) # error if not empty, or not found
        shutil.move(path+'/'+file,path+'/'+extension+'/'+file)
        shutil.copy("src", "dest")
        shutil.copy2("src", "dest")
        shutil.rmtree(input("folder name") ) # works for non empty directories
